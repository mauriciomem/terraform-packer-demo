...
build {
  ...
  # systemd unit for acme app
  provisioner "file" {
    source      = "templates/acmeapp.service"
    destination = "/tmp/acmeapp.service"
  }

  # configuration for acme app
  provisioner "file" {
    source      = "templates/acmeapp.env"
    destination = "/tmp/acmeapp.env"
  }
  ...
  # HCP Packer settings
  hcp_packer_registry {
    bucket_name = "acme-corp-image-mgmt"
    description = "This is an image for ACME corp."

    bucket_labels = {
      "acme-corp-project" = "acme-corp-frontend",
      "os-family"         = "Ubuntu",
